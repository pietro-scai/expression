...
#   2. Creates ~/.claude/plugins/marketplaces/sweet-dev/ pointing at
...
for skill_dir in "$SKILLS_SRC"/*/; do
  ...
  # sweet-framework: also wire up references so the agent can read the spec/docs
  if [[ "$name" == "sweet-framework" ]]; then
...
# --------------------------------------------------------------------------
# 2. Set up marketplace structure at ~/.claude/plugins/marketplaces/sweet-dev/
# --------------------------------------------------------------------------
...
ln -s "$REPO/plugins/claude" "$CC_MARKETPLACE/plugins/sweet"
echo "marketplace: sweet-dev → $CC_MARKETPLACE"
...
print("registered sweet-dev in", path)
PYEOF

echo ""
echo "Done. In Claude Code run:"
echo "  /plugin install sweet@sweet-dev"
