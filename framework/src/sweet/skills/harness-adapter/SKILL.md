...
`sweet agent` is harness-agnostic. The default `anthropic-api` harness
...
registered when `sweet agent` starts. That's it — no other surface area
to plug into.
...
- **Keep the harness pure.** All side effects (file writes, sweet runs)
