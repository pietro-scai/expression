...
def _resolve_model_cli() -> list[str]:
    ...
    collides with the user's ``sweet.py`` when cwd is the workspace."""
    for cand in ("sweet",):
        found = shutil.which(cand)
        if found:
            return [found]
    # Fallback: invoke via the package's __main__ but with a cwd-stripped
    # PYTHONPATH so the user's sweet.py doesn't shadow the installed package.
    return [sys.executable, "-m", "sweet"]
...
def _run_model(subcommand: str, extra: str, env: ToolEnv) -> str:
    ...
    # Strip cwd from PYTHONPATH so ``import sweet`` resolves to the installed
    # package, not the user's ``workspace/sweet.py``.
    import os as _os
