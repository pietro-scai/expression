"""``sweet doc sync`` — reconcile ``sweet.md`` with ``sweet.py`` (PRD §6).
...
class DocDrift:
    """Drift between code rows/scalars and ``sweet.md`` mentions."""
...
def format_drift(drift: DocDrift) -> str:
    if drift.empty:
        return "✓ sweet.md is in sync with sweet.py"
    lines: list[str] = []
    if drift.in_code_not_md:
        lines.append(
            f"# In code but not in sweet.md ({len(drift.in_code_not_md)})"
        )
        for name in drift.in_code_not_md:
            lines.append(f"  - {name}")
    if drift.in_md_not_code:
        lines.append(
            f"# Mentioned in sweet.md but not in code "
