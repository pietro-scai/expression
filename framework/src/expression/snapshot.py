"""Snapshot management and ``expression diff`` (Phase 3, PRD §10.2).

A snapshot is a pinned record of a previous solve. ``expression run`` writes the
...
when the user accepts current results via ``expression snapshot accept``.

``expression diff`` reports cell-level differences between the current solve and
