---
name: excel-fidelity
description: expression export must round-trip. If verification fails, surface the diff, don't paper over it.
---

`expression export` runs a verification step (PRD §9.2): the exported `.xlsx` is
...
4. Re-run `expression export`. Only declare success when verification passes.
