...
Two shapes live here:
  ...
  - ``to_json(model)`` mirrors the *solved* model: axes, glob values,
    ...
    scalars. Used by ``expression run`` → ``outputs/result.json``.

  - ``describe_model(model, md_text=...)`` mirrors the *defined* model:
    DAG between variables, per-row Python source + docstring + deps,
    column specs, optional expression.md documentation appended, and a
    mechanically generated Mermaid diagram. Used by ``expression describe``
    → ``outputs/model.json``.
...
def describe_models(models: list[Model], md_text: str | None = None) -> dict[str, Any]:
    ...
    Top-level keys:
      ...
      - ``documentation``: the contents of ``expression.md`` if provided.
