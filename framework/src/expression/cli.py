...
doc_app = typer.Typer(
    add_completion=False, no_args_is_help=True, help="Reconcile expression.md with expression.py."
)
...
def _load_module(model_path: Path) -> types.ModuleType:
    """Import ``expression.py`` and return the loaded module."""
    if not model_path.exists():
        raise typer.BadParameter(f"No expression.py found at {model_path}")
...
def _load_all_models(model_path: Path) -> list[Model]:
    """Load ALL Model subclasses from expression.py, sorted by cross-model dep order.
...
def init(
    ...
    (path / "outputs").mkdir()
    (path / "tests").mkdir()
    (path / "expression.py").write_text(_INIT_MODEL_PY.format(name=name, classname=classname))
    (path / "expression.md").write_text(
...
def run(
    ...
    model_path: Path = typer.Option(
        Path("expression.py"), "--model", help="Path to the expression.py file."
    ),
) -> None:
    """Solve all models in expression.py and write outputs/result.json.
...
def show(
    ...
    argument: ``expression show 'budget[2024]'``.

    With multiple models in the file you can qualify the row name:
    ``expression show 'SalaryModel.gross_salary[2025]'``.
    """
    try:
        models = _load_all_models(model_path)
        for model in models:
...
def export_cmd(
    ...
    model_path: Path = typer.Option(Path("expression.py"), "--model", help="Path to expression.py."),
...
def import_cmd(
    ...
    """Convert ``.xlsx`` → ``expression.py`` + ``expression.md`` (PRD §9.1).

    Phase 2 is non-interactive: produces a Layer-1 skeleton with TODOs in
    ``expression.md`` for ambiguities. Phase 3 wires this into the agent loop.
    """
    ...
    typer.echo(
        ...
        f"{len(plan.globs)} glob(s) → {out}/expression.py + expression.md"
    )
    if plan.issues:
        typer.echo("Issues found (also recorded in expression.md):")
...
def describe(
    ...
    Unlike ``expression run`` (which writes solved values to result.json), this
    ...
    mechanically generated Mermaid diagram. ``expression.md`` is appended as
    ...
    md_path = model_path.parent / "expression.md"
...
def explain(
    ...
    model_path: Path = typer.Option(
        Path("expression.py"), "--model", help="Path to expression.py."
    ),
) -> None:
...
def overrides_add(
    ...
    model_path: Path = typer.Option(Path("expression.py"), "--model"),
) -> None:
...
def doc_sync(
    ...
    """Report drift between ``expression.py`` and ``expression.md`` (PRD §6).
    ...
    that don't resolve to code. Use ``expression agent`` for interactive
    reconciliation.
    """
    ...
    if not md_path.exists():
        typer.echo(f"✗ No expression.md found at {md_path}", err=True)
        raise typer.Exit(1)
    try:
        model = _load_user_model(model_path)
    except ModelError as exc:
