...
framework operator-manual skill (`sweet-framework`), and the long-form
...
```
amsterdam/
  src/sweet/skills/                  # SOURCE OF TRUTH (nine skills)
    ...
    sweet-framework/SKILL.md         # discipline + CLI surface + workspace conventions
  ...
  plugins/
    claude/
      ...
      commands/                      # /sweet:run, /sweet:diff, …
      skills/                        # ← materialised from src/sweet/skills/
...
`plugins/shared/skills-link.sh` from `src/sweet/skills/` and the top-level
docs.
...
The plugins layer is additive: nothing in the existing `sweet` CLI is
...
The plugin name `sweet` becomes the prefix for slash commands and skills in
Claude Code (`/sweet:bottom-up-modeling`, `/sweet:run`, …).
...
- **`sweet-framework` skill** — the framework's operator manual, written
  ...
  `src/sweet/skills/sweet-framework/SKILL.md` and is propagated by
  `make plugins`.
- **`reference/` bundle** — `SPECIFICATION.md` (formal spec) and
  `docs/DOCS.md` (long-form tutorial) are mirrored into each plugin under
  `plugins/{claude,codex}/reference/`. The `sweet-framework` skill points
  the in-host agent at these files when it needs authoritative answers.

The user still installs the `sweet` CLI separately (`uv tool install
...
- Nine skills available (loaded automatically when relevant based on their
  `description` field): `bottom-up-modeling`, `sweet-framework`, etc.
- Slash commands prefixed `/sweet:` — `/sweet:run`, `/sweet:diff`,
  `/sweet:show`, `/sweet:explain`, `/sweet:export`.

The `sweet-framework` skill carries the discipline rules + CLI surface;
the eight content skills layer specific guidance on top. The
`sweet-framework` skill also bundles `references/SPECIFICATION.md` and
...
If `/sweet:run` solves the model and `/sweet:diff` shows a meaningful
...
common `sweet` CLI invocations. Each file is a Claude Code slash-command:
the body instructs Claude what to do, and Claude calls the framework via
its `Bash` tool.

| Command | Wraps | Purpose |
|---|---|---|
| `/sweet:run` | `sweet run` | Solve the workspace and print the row table; report deltas vs the last snapshot. |
| `/sweet:diff` | `sweet diff` | Show the cell-level diff against the accepted snapshot. |
| `/sweet:show` | `sweet show <cell>` | Inspect one cell's value and provenance. |
| `/sweet:explain` | `sweet explain <cell>` | Walk a cell's dependency tree and show the formula path. |
| `/sweet:export` | `sweet export` | Round-trip the model out to `.xlsx`. |
...
**Status: not shipped in v1.** The `sweet-framework` skill plus the
...
The plan leaves room for a `.mcp.json` exposing `sweet run/show/diff/…`
...
`sweet init` → `sweet run` → tweak → `sweet run` without rough edges.
